...
void handle_rf_rx_packet(uint8_t *Rx_buffer, size_t len) {
	...
	memcpy(&sender_hardware_id, &Rx_buffer[2], 4);
	memcpy(&receiver_hardware_id, &Rx_buffer[6], 4);
	...
	memcpy(&incoming_crc32, &Rx_buffer[10 + payload_len], 4);
	...
	handle_payload_data(identifier, &Rx_buffer[10]);
}
