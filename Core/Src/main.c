...
void handle_payload_data(uint8_t identifier, uint8_t *payload_data) {
	...
	case FIRE_DROGUE_REQ:
		...
		if (state_machine_fc.drogue_arm_state == ARMED) {
	...
	case FIRE_MAIN_REQ:
		...
		if (state_machine_fc.main_arm_state == ARMED) {
	...
	case ARM_DROGUE_REQ:
		...
		arm_drogue_res arm_drogue_response = { .arm_drogue_state = state_machine_fc.drogue_arm_state };
	...
	case ARM_MAIN_REQ:
		...
		arm_main_res arm_main_response = { .arm_main_state = state_machine_fc.main_arm_state };
...
void State_Machine(void *argument)
{
  /* USER CODE BEGIN State_Machine */
	...
	while (1) {
		...
		if (!result) {
			...
			if (angle_from_vertical <= DEG_TO_RAD(PITCH_OVER_ANGLE_THRESHOLD) || M_PI - angle_from_vertical <= DEG_TO_RAD(PITCH_OVER_ANGLE_THRESHOLD)) {
				...
				updateMedianFilter(&launch_median_filter, acceleration_magnitude, (float) HAL_GetTick() / 1000.0);
	...
	initExpLowPassFilter(&altitude_exp_lp_filter, VERTICAL_VELOCITY_DETECT_FREQ, ALTITUDE_LP_FILTER_CUTOFF_FREQ, ms5611_data.altitude);
	...
	while (1) {
		...
		float time_since_launch = (float) (pdMS_TO_TICKS(xTaskGetTickCount()) * portTICK_PERIOD_MS - state_machine_fc.launch_time) / 1000.0;
		...
		if ((float) (pdMS_TO_TICKS(xTaskGetTickCount()) * portTICK_PERIOD_MS - last_velocity_calculation_time) / 1000.0 >= (1.0 / VERTICAL_VELOCITY_DETECT_FREQ)) {
			...
			float filtered_altitude = updateExpLowPassFilter(&altitude_exp_lp_filter, ms5611_data.altitude);
